...
import { updateFee, deleteFee, getExpenseDetails, updateFeeByInputQty, updateAPFee, copyAPFee } from 'common/reducers/cmsExpense';
...
function createFieldLabelMap(msg) {
  fieldLabelMap.orig_amount = msg('origAmount');
  fieldLabelMap.currency = msg('origCurrency');
  fieldLabelMap.exchange_rate = msg('exchangeRate');
  fieldLabelMap.remark = msg('remark');
  fieldLabelMap.base_amount = msg('baseAmount');
  fieldLabelMap.invoice_party = msg('invoiceParty');
  fieldLabelMap.invoice_cat = msg('invoiceCat');
  fieldLabelMap.fee_invoice_no = msg('invoiceNo');
  fieldLabelMap.tax = msg('plusTax');
}
...
@connect(
  state => ({
    ...
    eventFeeClear: state.cmsExpense.eventFeeClear,
  }),
  {
    updateFee,
    deleteFee,
...
export default class FeesPane extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.eventFeeClear.status) {
      const { events } = nextProps.eventFeeClear;
      const dataSource = [...this.state.dataSource];
      const { feeEventMap } = this.props;
      for (let i = 0; i < events.length; i++) {
        const event = events[i];
        const eventData = dataSource.filter(data => data.fee_type !== 'SP' && feeEventMap[data.fee_code] === event);
        for (let j = 0; j < eventData.length; j++) {
          const data = eventData[j];
          data.base_amount = null;
          data.orig_amount = null;
          data.recvable_account = 0;
          data.recvable_other = 0;
          data.payable_account = 0;
          data.payable_other = 0;
          data.sum_amount = null;
          data.remark = null;
          data.invoice_party = null;
          data.invoice_cat = null;
          data.fee_invoice_no = null;
          data.tax = null;
          data.adv_tenant_id = null;
        }
      }
      this.setState({
        dataSource,
  ...
  columns = [{
    ...
    render: (o, record) => {
      ...
      if (record.fee_status < 2) {
        if (this.state.editItem.id === record.id) {
          return (<span>
            <PrivilegeCover module="clearance" feature="billing" action="edit">
  ...
  handleOk = () => {
    ...
    const {
      tenantId, currencies, tabName, delgNo,
    } = this.props;
    const dataSource = [...this.state.dataSource];
    ...
    const contentLog = [];
    if (item.id) {
      ['orig_amount', 'exchange_rate', 'currency', 'remark', 'base_amount', 'invoice_party',
        'invoice_cat', 'fee_invoice_no', 'tax'].forEach((field) => {
        if (dataSource[index][field] !== item[field] &&
          !(!dataSource[index][field] && !item[field])) {
          if (field === 'currency') {
            const value = currencies.find(curr => curr.currency === item[field]) &&
              currencies.find(curr => curr.currency === item[field]).name;
            const oldValue = currencies.find(curr => curr.currency === dataSource[index][field]) &&
              currencies.find(curr => curr.currency === dataSource[index][field]).name;
            contentLog.push(`"${fieldLabelMap[field]}"由 [${oldValue || ''}] 改为 [${value || ''}]`);
          } else {
            contentLog.push(`"${fieldLabelMap[field]}"由 [${dataSource[index][field] || ''}] 改为 [${item[field] || ''}]`);
          }
        }
      });
    }
    if (item.id && item.fee_type !== BSS_FEE_TYPE[1].key) {
      prom = this.props.updateFee(
        ...
        contentLog.length > 0 ? `修改 "${tabName}" 费用代码[${item.fee_code}] ${contentLog.join(';')}` : '',
        delgNo,
      );
  ...
  handleDelete = (row) => {
    ...
    this.props.deleteFee(row.id, `删除"${tabName}"费用代码[${row.fee_code}]`);
    const dataSource = [...this.state.dataSource];
    const index = dataSource.findIndex(data => data.id === row.id);
    dataSource.splice(index, 1);
  ...
  handleAddSpecial = () => {
    const { expense, type, tabName } = this.props;
    let feeCodes = [];
    ...
    this.props.toggleAddSpecialModal(
      true,
      {
        feeCodes, tabName, expenseNo: expense.expense_no, expenseType: type,
      }
    );
  }
