...
import { toggleFeesWriteInModal, getBillingFeesByBizNo, updateAPFee, updateFee } from 'common/reducers/tmsExpense';
...
@connect(
  state => ({
    visible: state.tmsExpense.feesWriteInModal.visible,
    selFeeCodes: state.tmsExpense.feesWriteInModal.selFeeCodes,
    billingFees: state.tmsExpense.feesWriteInModal.billingFees,
  }),
  {
...
export default class BatchFeeWriteInModal extends React.Component {
        ...
        this.props.getBillingFeesByBizNo(
          ...
          if (!result.error) {
            ...
            expenseFeesMap[expNo] = {
              ...
              custOrderNo: result.data.custOrderNo,
              shipmtNo: result.data.shipmtNo,
            };
  ...
  render() {
    ...
    const headInfo = selFeeCodes.reduce(
      ...
      { mainHead: ['订单追踪号', '运输编号'], subHead: ['', ''], index: 2 }
    );
    ...
    for (let i = 0; i < expenses.length; i++) {
      ...
      const { fees, shipmtNo } = expenseFeesMap[expNo];
      const body = [`${expenseFeesMap[expNo].custOrderNo || ''}`, shipmtNo];
